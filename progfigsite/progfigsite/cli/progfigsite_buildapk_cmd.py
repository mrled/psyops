...
def main():
    ...
    default_pyproject_root = Path(progfigsite.__file__).parent.parent
    parser.add_argument(
        "--pyproject-root",
        type=Path,
    ...
    progfigsite_package_root = Path(progfigsite.__file__).parent
    ...
    with tempfile.TemporaryDirectory() as tmpdir_str:
        ...
        tmp_pyproj_root: Path = tmpdir / parsed.abuild_repo_name / parsed.pyproject_root.name
        tmp_pyproj_root.mkdir(parents=True)
        shutil.copytree(
            parsed.pyproject_root,
            tmp_pyproj_root,
            ignore=shutil.ignore_patterns("*venv*", ".git*", "*.egg-info", ".mypy_cache", "build", "dist"),
            dirs_exist_ok=True,
        )
        ...
        with ProgfigsitePythonPackagePreparer(tmp_package_root) as preparer:
            ...
            magicrun(
                ["abuild", "-P", parsed.apks_index_path.as_posix(), "-D", parsed.abuild_repo_description],
